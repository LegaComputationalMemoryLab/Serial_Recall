"""
SR1 -- Serial Recall Task (C-Pod trigger variant)
...
# C-Pod support is optional -- without it the task runs in test mode
# (no EEG triggers; pulse timestamps are still logged).
try:
    import pyxid2
    PYXID_AVAILABLE = True
except Exception:
    PYXID_AVAILABLE = False
    print("pyxid2 not available -- running WITHOUT C-Pod triggers (test mode)")
...
# Define the cpod trigger legend
def cpod_trigger_legend():
    return {
        "Sync": 1   # DC02
    }

# Auto-detect C-Pod port and return cpod device object
def find_cpod_port():
    devices = pyxid2.get_xid_devices()
    if devices and devices[0] is not None:
        print("Found C-pod device")
        return devices[0]
    raise RuntimeError("Unable to find the C-pod, is it plugged in?")

# Send the cpod trigger based on the label
def send_sync_pulse(dev, trigger_label):
    trigger_map = cpod_trigger_legend()
    trigger_id = trigger_map.get(trigger_label)

    if not trigger_id:
        raise ValueError(f"Trigger label {trigger_label} not recognized. Please add to cpod_trigger_legend()")

    if dev is not None:
        dev.set_pulse_duration(50)  # pulse duration 50ms
        dev.activate_line(trigger_id)
...
# Connect to the C-Pod (None in test mode)
cpod = find_cpod_port() if PYXID_AVAILABLE else None
...
# Usage: only meaningful when the C-Pod is connected
if PYXID_AVAILABLE:
...
# Show logo (cropped file is 674x609; display at the same aspect ratio)
...
for list_num in range(1, n_lists + 1):
    ...
    for trial_index, trial in enumerate(trials, start=1):
        ...
        encoding_pulse = send_sync_pulse(cpod, "Sync")
    ...
    for probText, correctIsTrue in problems:
        ...
        pulse_time = send_sync_pulse(cpod, "Sync")
    ...
    try:
        mic.start()
        rec_start = send_sync_pulse(cpod, "Sync")   # aligns audio t=0 to the EEG record
        recording = True
    ...
    beep.play()
    beep_time = send_sync_pulse(cpod, "Sync")
